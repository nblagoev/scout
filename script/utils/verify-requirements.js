...
function verifyNode(cb) {
  ...
  if (nodeMajorVersion === 0 && nodeMinorVersion < 10) {
    error = "node v0.10 is required to build Scout, node " + nodeVersion + " is installed.";
    cb(error);
  }
  else {
...
function verifyNpm(cb) {
  ...
  childProcess.execFile(npmCommand, ['-v'], { env: process.env }, function (err, stdout) {
    if (err)
      return cb("npm 1.4 is required to build Scout. An error (" + err + ") occured when checking the version.");
    ...
    if (npmMajorVersion === 1 && npmMinorVersion < 4)
      cb("npm v1.4+ is required to build Scout.");
    else
...
function checkPythonVersion (python, cb) {
  ...
  childProcess.execFile(python, ['-c', 'import platform; print(platform.python_version());'], { env: process.env }, function (err, stdout) {
    if (err) {
      error = "Python 2.7 is required to build Scout. An error (" + err + ") occured when checking the version of '" + python + "'. ";
    ...
    // Scout requires python 2.7 or higher (but not python 3) for node-gyp
    ...
    if (!goodPythonVersion) {
      error = "Python 2.7 is required to build Scout. '" + python + "' returns version " + version + ". ";
      error += pythonHelpMessage;
      cb(error);
      return;
    }
