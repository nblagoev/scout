...
export default class HttpEnvelope {
  ...
  set inProgress(value) {
    ...
    if (!value && !global.scout.currentWindow.isFocused()) {
      global.scout.currentWindow.flashFrame(true)
    }
  }
  ...
  sendRequest(callback) {
    ...
    request(this.request.toJSON(), (error, res, body) => {
      if (error) {
        ...
        global.scout.notifications.addError(message, {detail})
      } else {
...
class HttpEnvelopeEntity {
   ...
   *        any of the {HttpEnvelopeEntity}'s properties.
   *
  ...
  findHeader(predicate) {
    for (let [index, header] of this.headers.entries()) {
      if (typeof predicate === 'function' &&
          ...
          header.name.toLowerCase() === predicate.toLowerCase()) {
        return { header, index }
      }
    }
...
class HttpRequest extends HttpEnvelopeEntity {
  constructor() {
    super()
...
class HttpResponse extends HttpEnvelopeEntity {
  constructor() {
    super()
    this.body = null
    this.status = 0
...
class HttpRequestBody {
  ...
  serialize() {
    ...
    if (this.type === 'text') {
      ...
      global.scout.notifications.addError(
        'Cannot serialize body', {detail: `Unsupported body type: ${this.type}`})
    }
  ...
  deserialize(bodyObj) {
    if (bodyObj.type === 'text') {
      ...
      global.scout.notifications.addError(
        'Cannot deserialize body', {detail: `Unsupported body type: ${bodyObj.type}`})
    }
  }
  ...
  toString() {
    if (this.type === 'text') {
      ...
      global.scout.notifications.addError(`Unsupported body type: ${this.type}`)
    }
  }
}
...
class UrlEncodedHttpRequestBody {
  ...
  serialize() {
    return this.pairs.map(p => ({ key: p.key, value: p.value, include: p.include }))
  }
...
class HttpEnvelopeKeyValuePair {
...
class HttpHeader extends HttpEnvelopeKeyValuePair {}

class HttpParameter extends HttpEnvelopeKeyValuePair {}
